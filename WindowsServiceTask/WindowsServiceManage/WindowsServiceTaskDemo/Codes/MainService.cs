            ...
            try {
                ...
                try {
                    ...
                    foreach (var groupName in scheduler.GetJobGroupNames()) {
                        foreach (JobKey jobKey in scheduler.GetJobKeys(GroupMatcher<JobKey>.GroupEquals(groupName))) {
                            String jobName = jobKey.Name;
                            String jobGroup = jobKey.Group;
                            List<ITrigger> triggers = (List<ITrigger>)scheduler.GetTriggersOfJob(jobKey);
